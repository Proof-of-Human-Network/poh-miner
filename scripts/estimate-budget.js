#!/usr/bin/env node
/**
 * Estimate the μDAI budget (bid) for a compute / chat job.
 *
 * Pricing model (see src/jobs/gas-estimator.js):
 *   cost   = tokens × gasPrice        gasPrice = 1 μDAI per AI token (1 DAI = 1e9 tokens)
...
const prompt   = process.argv[2] || process.env.PROMPT || 'In one sentence, what is decentralized artificial intelligence?';
...
const dai = (u) => (u / 1e9).toFixed(9) + ' DAI';

console.log(`Prompt chars       : ${prompt.length}`);
console.log(`Prompt tokens (~)  : ${promptTokens}`);
console.log(`gasPrice           : ${gasPrice} μDAI/token`);
console.log(`Reserved output    : ${Math.min(maxOut, GAS.OUTPUT_CAP)} tokens (cap ${GAS.OUTPUT_CAP})`);
console.log('──────────────────────────────────────────────');
console.log(`Floor (min bid)    : ${floor} μDAI  (${dai(floor)})  → 0 output tokens`);
console.log(`Recommended bid    : ${budget} μDAI  (${dai(budget)})`);
...
if (process.env.BUDGET) {
  ...
  console.log(`Reverse: BUDGET=${b} μDAI (${dai(b)}) → up to ${out} output tokens` +
