...
| Topic | Reference |
|-------|-----------|
| Touch Targets, Safe Area, CollectionView | `layout system` (reference not bundled) |
| TabBar, NavigationController, Modal | `navigation patterns` (reference not bundled) |
| StackView, Button, Alert, Search, ContextMenu | `uikit components` (reference not bundled) |
| CAShapeLayer, CAGradientLayer, Core Animation | `graphics animation` (reference not bundled) |
| Dynamic Type, Semantic Colors, VoiceOver | `accessibility` (reference not bundled) |
| Permissions, Location, Share, Lifecycle, Haptics | `system integration` (reference not bundled) |
| Metal Shaders & GPU | `metal shader` (reference not bundled) |
| SwiftUI HIG, Components, Patterns, Anti-Patterns | `swiftui design guidelines` (reference not bundled) |
| Optionals, Protocols, async/await, ARC, Error Handling | `swift coding standards` (reference not bundled) |
...
`skills/ios-application-dev/SKILL.md` from `MiniMax-AI/skills` with the `gitmcp` MCP.
