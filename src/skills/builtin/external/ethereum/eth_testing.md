...
**You don't fork-test.** If your contract calls Uniswap, Aave, or any external protocol (verified addresses: `eth_addresses` skill), test against their real deployed contracts on a fork. Mocking them hides integration bugs that only appear with real state.
...
contract SwapTest is Test {
    // Real mainnet addresses — full verified list: `eth_addresses` skill
