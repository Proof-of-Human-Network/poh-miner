 ...
 * Real DAI Adapter
 *
 * This bridges the miner network to the EXISTING DAI codebase
 * located at ../../dev (the original iamai.kg stack).
 *
...
async function loadRealDAIModules() {
  if (loaded) return;

  if (!process.env.BRAIN_DATA_DIR) {
    const brainDir = path.join(os.homedir(), '.dai-miner', 'brain');
    fs.mkdirSync(brainDir, { recursive: true });
    process.env.BRAIN_DATA_DIR = brainDir;
    console.log(`[RealDAI] Brain state dir: ${brainDir}`);
  }
  ...
  try {
    ...
    console.log('[RealDAI] Loaded embedded checker + brain');
    console.log('[RealDAI] Network signals: hash=' + methodsManager.hash + ', count=' + methodsManager.getActiveMethods().length);
    loaded = true;
  } catch (err) {
    console.error('[RealDAI] Failed to load checker:', err.message);
    loaded = true;
  }
}

export async function computeWithRealDai(job, config) {
  ...
  // Full DAIMinerNode does this early, but standalone compute / tests / early jobs need it too to avoid
  ...
  try {
    ...
    if (config && config.rpc && Object.keys(config.rpc).length > 0) {
      process.env.DAI_RPC_CONFIG = JSON.stringify(config.rpc);
    }
  } catch (e) {
    console.warn('[RealDAI] Could not apply RPC config for checker/signals:', e.message);
  }

  await loadRealDAIModules();

  if (!checker || typeof checker.runFullCheck !== 'function') {
    console.warn('[RealDAI] runFullCheck not available from checker module. Using simulation.');
  ...
  if (!ADDRESS_RE.test(address)) {
    if (DOMAIN_RE.test(address)) {
      ...
      if (resolved) {
        console.log(`[RealDAI] Domain resolved: ${address} → ${resolved}`);
        scanAddress = resolved;
      } else {
        console.warn(`[RealDAI] Could not resolve domain "${address}" — scanning as-is`);
      }
    } else {
      ...
      if (USERNAME_RE.test(ihQuery)) {
        ...
        if (ihResult) {
          console.log(`[RealDAI] IdentityHub resolved "${address}" → ${ihResult.address}`);
          scanAddress = ihResult.address;
          identityHubAgentData = ihResult.agentData;
        } else {
          console.warn(`[RealDAI] IdentityHub: no address found for "${address}"`);
        }
      }
    }
  }
  ...
  try {
    ...
    return {
      ...
      reasoning: fullResult.reasoning || 'Computed with real DAI brain + signals',
    ...
    console.error('[RealDAI] Real computation failed:', err.message);
    const sim = simulateVerdict(job, config, methodsManager);
    sim.reasoning = `[Fallback] Real DAI error: ${err.message}`;
    return sim;
  }
}

// Brain API — accessible after loadRealDAIModules() has run
...
export function getBrainDataDir() {
  ...
  return path.join(os.homedir(), '.dai-miner', 'brain');
}
...
function simulateVerdict(job, config, mgr = null) {
  ...
  return {
    ...
    reasoning: 'Computed using real DAI logic (simulation mode)',
